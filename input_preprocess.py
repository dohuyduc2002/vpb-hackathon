...
def preprocess_single(sample, node_mapping, scaler, label_encoders):
    ...
    merchant_city = label_encoders["merchant_city"].transform([sample["merchant_city"]])[0] if sample["merchant_city"] else 0
    use_chip = label_encoders["use_chip"].transform([sample["use_chip"]])[0] if sample["use_chip"] else 0
    errors = label_encoders["errors"].transform([sample["errors"]])[0] if sample["errors"] else 0
    mcc = label_encoders["mcc"].transform([sample["mcc"]])[0] if sample["mcc"] else 0
    merchant_name = label_encoders["merchant_name"].transform([sample["merchant_name"]])[0] if sample["merchant_name"] else 0
