...
def calculate_pr_auc_with_thresholds(
    y_true, y_pred_proba, thresholds=[0.5, 0.6, 0.7, 0.8]
):
    """
    ...
    return {"pr_auc": pr_auc, "threshold_metrics": threshold_metrics}
...
class SimplifiedHybridFraudDetector:
    ...
    def create_graph_features(self, df):
        ...
        merchant_features.columns = [
            "merchant_" + "_".join(col).strip() for col in merchant_features.columns
        ]
        ...
        enhanced_df = enhanced_df.merge(
            merchant_features, left_on="Merchant_ID", right_index=True, how="left"
        )

        enhanced_df["amount_vs_user_avg"] = enhanced_df["Amount"] / (
            enhanced_df["Amount_mean"] + 1e-6
        )
        enhanced_df["amount_vs_merchant_avg"] = enhanced_df["Amount"] / (
            enhanced_df["merchant_Amount_mean"] + 1e-6
        )
        ...
        enhanced_df["is_night"] = (
            (enhanced_df["Hour"] < 6) | (enhanced_df["Hour"] > 22)
        ).astype(int)
        enhanced_df["high_amount"] = (
            enhanced_df["Amount"] > enhanced_df["Amount"].quantile(0.95)
        ).astype(int)
    ...
    def prepare_features(self, df):
        ...
        feature_columns = [
            ...
            "Hour_<lambda>",
            "MCC_nunique",
            "merchant_Amount_count",
            "merchant_Amount_mean",
            "merchant_Amount_std",
            "merchant_Is_Fraud_mean",
            "merchant_User_ID_nunique",
            "amount_vs_user_avg",
    ...
    def train(self, df, test_size=0.2):
        ...
        for threshold in [0.5, 0.6, 0.7, 0.8]:
            ...
            print(
                f"   {threshold:.1f}    |   {metrics['precision']:.3f}   | {metrics['recall']:.3f} |  {metrics['f1_score']:.3f}  | {metrics['predicted_positive']:4d} | {metrics['prediction_rate']:.3f}"
            )
...
def load_and_preprocess_encoded_csv(csv_path, max_samples=500000):
    ...
    if len(df) > max_samples:
        print(
            f"Dataset too large ({len(df)} rows). Sampling {max_samples} transactions..."
        )
        ...
        df = (
            pd.concat([sampled_fraud, sampled_normal])
            .sample(frac=1, random_state=42)
            .reset_index(drop=True)
        )
        print(
            f"Sampled {len(df)} transactions (fraud rate: {df['Is Fraud?'].mean():.3f})"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train simplified hybrid fraud detection model"
    )
    parser.add_argument(
        "--csv_path", type=str, default="./data.csv", help="Path to encoded CSV file"
    )
    parser.add_argument(
        "--model_dir",
        type=str,
        default="./simple_hybrid_model",
    ...
    try:
        ...
        df = load_and_preprocess_encoded_csv(
        ...
        for threshold in [0.5, 0.6, 0.7, 0.8]:
            ...
            print(
                f"  Threshold {threshold}: Precision={metrics['precision']:.3f}, Recall={metrics['recall']:.3f}, F1={metrics['f1_score']:.3f}"
            )
