...
data_train, data_test = train_test_split(
    list(range(data.edge_index.shape[1])),
    test_size=0.2,
    random_state=42,
    stratify=labels,
)
...
class GATEncoderWithEdgeAttrs(nn.Module):
    def __init__(
        self, in_channels, hidden_channels, edge_attr_dim, heads=4, dropout=0.3
    ):
        super().__init__()
...
encoder = GATEncoderWithEdgeAttrs(
    in_channels=x.shape[1], hidden_channels=64, edge_attr_dim=edge_attr.shape[1]
).to(device)

model = GAE(encoder).to(device)

classifier = EdgeMLPClassifier(emb_dim=64, hidden_dim=64, num_classes=2).to(device)
...
for epoch in range(500):
    ...
    acc_train = (
        pred_train == data.y[train_mask]
    ).sum().item() / train_mask.sum().item()
    ...
    with torch.no_grad():
        ...
        acc_test = (
            pred_test == data.y[test_mask]
        ).sum().item() / test_mask.sum().item()
    ...
    if epoch % 50 == 0:
        print(
            f"Epoch {epoch+1}: Train Loss={loss.item():.4f}, Acc={acc_train:.4f} | Test Loss={loss_test.item():.4f}, Acc={acc_test:.4f}"
        )
